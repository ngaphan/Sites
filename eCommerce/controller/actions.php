...
if ($menu === 'home')
{
    // On instancie les objets $panierManager,
    // pour pouvoir lister et créer nos paniers sur la page d'accueil
   ...
   // $prixHTManager = new PrixHTManager($PDO);
   // $produitManager = new ProduitManager($PDO);
   // $tauxTVAManager = new TauxTVAManager($PDO);


    // Pour chaque sélection nulle (= l'utilisateur n'a pas encore sélectionné sur une des
    //listes),
    ...
    if (!isset($_SESSION['selectedRefProduit'])) $_SESSION['selectedRefProduit'] = 0;
   ...
   // if (!isset($_SESSION['selectedWeaponId'])) $_SESSION['selectedWeaponId'] = 0;
    ...
    {
        // S'il existe bien un dragon ayant cet identifiantarm
        if (isset($_GET['refProduit']) && $dragonManager->exists($_GET['refProduit']))
        {
            // On enregistre la sélection de ce dragon dans la session / $_SESSION
            $_SESSION['selectedRefProduit'] = $_GET['refProduit'];
        }
    ...
    if ($_SESSION['selectedDragonId'] != 0 )
    {
    ...
    $produits = $produitManager->listAll();
